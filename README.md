...
**-s seqfile** : The input sequence file, adhering to PHYLIP conventions. Can be sequential or interleaved.
...
**-ri** : Drop invariant sites from the sequence data before processing. **DEFAULT:** Don’t drop invariant sites.
...
allows multiple instances of the program to run at the same time, as they would not overwrite each other’s files.
Note: Some temporary files generated by dgpe do not have suffixes added to them, so it is not recommended to
...
It is recommended to first install MAMMaL following the instructions in Susko (2022) (see this [link](https://www.mathstat.dal.ca/~tsusko/doc/mammal.pdf)), and the R package phangorn. MEOW comes with three files:
